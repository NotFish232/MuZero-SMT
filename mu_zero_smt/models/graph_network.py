...
class GraphEncoder(nn.Module):
    def __init__(
        ...
        self.pool = TGnn.aggr.AttentionalAggregation(
    ...
    def forward(
        self: Self, x: T.Tensor, edge_index: T.Tensor, batch: T.Tensor
    ) -> T.Tensor:
        x = self.input_proj(x)
...
class GraphNetwork(MuZeroNetwork):
    ...
    def __init__(
        ...
        representation_num_layers: int,
        dynamics_layers: list[int],
        dynamics_heads_layers: list[int],
        policy_layers: list[int],
        policy_discrete_head_layers: list[int],
        policy_continuous_heads_layers: list[int],
        ...
        self.dynamics_heads = nn.ModuleList(
            [
                mlp(
                    self.encoded_state_size + dim,
                    dynamics_heads_layers,
                    self.encoded_state_size,
                )
        ...
        self.policy_discrete_head = mlp(
            self.encoded_state_size,
            policy_discrete_head_layers,
            len(self.action_space),
        )
        self.policy_continuous_heads = nn.ModuleList(
            [
                mlp(self.encoded_state_size, policy_continuous_heads_layers, 2 * dim)
                for dim in self.action_space
            ]
        )
        ...
        self.value_model = mlp(
            self.encoded_state_size, value_layers, self.full_support_size
        )

        # Reward model
        self.reward_model = mlp(
            self.encoded_state_size, reward_layers, self.full_support_size
        )
    ...
    def dynamics(
        ...
        for cur_action, (head, dim) in enumerate(
            ...
            if action_mask.any():
                ...
                # Residual connection
                next_encoded_state[action_mask] = head(batch) + batch_states
